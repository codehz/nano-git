/**
 * Virtual Worktree dirty-dir summary 重建策略
 *
 * 把 worktree 写路径里的脏目录摘要清理与重建逻辑集中到单独模块，
 * 让 worktree.ts 更接近纯编排层。
 */
...
import { getRootNode } from "./worktree-path.ts";
...
export function createDirtyDirPlanner(
  ...
  return {
    ...
    rebuild(touchedPaths): void {
      ...
      const visitDirectory = (node: WorktreeNode, dirPath: string): number => {

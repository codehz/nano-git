 ...
 * v2 Upload-Pack 服务
 *
 * 聚合 v2 协议服务端的能力广告和命令处理。
 */
export interface V2UploadPackService {
  /**
   * 生成能力广告
   *
   * @param service - 服务类型
   ...
   * @throws {V2ServeError} 命令不支持或参数不合法
   */
  handleCommand(body: Buffer): Buffer;
}

/**
 * 创建 v2 Upload-Pack 服务
 *
 * @param backend - 仓库后端
 * @returns V2UploadPackService 实例
 *
 * @example
 * ```ts
 ...
 * ```
 */
export function createV2UploadPackService(backend: RepositoryBackend): V2UploadPackService {
  return {
    advertise(service: string): Buffer {
    ...
    handleCommand(body: Buffer): Buffer {
      ...
      switch (parsed.command) {
        ...
        default: {
          throw new V2ServeError(`unknown command: ${parsed.command}`);
        }
      }
    },
  };

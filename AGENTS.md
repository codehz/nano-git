...
- **子路径注册方式**：添加新的子路径入口时，**只需在 `tsdown.config.ts` 的 `entry` 数组中添加入口文件路径**，然后运行 `bun run build`，`tsdown` 会自动更新 `package.json` 的 `exports` 和 `publishConfig.exports`。**不要手动编辑 `package.json` 中的导出映射**，否则下次 build 会被覆盖。

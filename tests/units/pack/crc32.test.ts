...
describe("crc32Value()", () => {
  test("空数据的 CRC32", () => {
    const crc = crc32Value(bytes(""));
    // CRC32 of empty data = 0
    expect(crc).toBe(0);
  });

  test("已知数据的 CRC32 值", () => {
    const crc = crc32Value(bytes("hello"));
    // 已知值验证
  ...
  test("不同数据产生不同 CRC32", () => {
    const a = crc32Value(bytes("data1"));
    const b = crc32Value(bytes("data2"));
    expect(a).not.toBe(b);
  });
  ...
  test("与 Git 的 CRC32 算法兼容", () => {
    ...
    // 验证已知值
    const crc = crc32Value(bytes("blob 11\u0000hello world"));
  ...
  test("查找表缓存正常工作", () => {
    // 多次调用应使用缓存的查找表
    const crc1 = crc32Value(bytes("a"));
    const crc2 = crc32Value(bytes("a"));

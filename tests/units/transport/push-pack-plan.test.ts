...
describe("computeObjectsToSend", () => {
  test("推送本地新增对象应全部包含", () => {
    ...
    const localHash = writeObject(store, { type: "blob", content: bytes("new content") });
  ...
  test("远程已有的对象应被排除", () => {
    const store = createMemoryObjectStore();
    const sharedHash = writeObject(store, { type: "blob", content: bytes("shared") });
    const localOnlyHash = writeObject(store, { type: "blob", content: bytes("local only") });
  ...
  test("删除操作（localHash === null）跳过对象收集", () => {
    ...
    writeObject(store, { type: "blob", content: bytes("deleted content") });

 ...
 * @param content - 帧内容
 */
function sideBandPkt(channel: number, content: string | Uint8Array): Uint8Array {
  const payload = typeof content === "string" ? utf8ToBytes(content) : content;
 ...
 */
function sideBandEmptyPkt(channel: number): Uint8Array {
  return encodePktLine(Uint8Array.from([channel]));
}
...
describe("decodeReceivePackResponse()", () => {
  ...
  test("side-band channel 1：report-status", () => {
    ...
    const data = concatBytes(sideBandPkt(0x01, reportStatus), encodeFlushPkt());
    const result = decodeReceivePackResponse(data);
  ...
  test("side-band channel 3：fatal error", () => {
    const data = concatBytes(sideBandPkt(0x03, "fatal: pack too large\n"), encodeFlushPkt());
  ...
  test("空数据返回空结果", () => {
    const result = decodeReceivePackResponse(allocBytes(0));
  ...
  test("side-band channel 1 跨多帧拼接 report-status", () => {
    ...
    const part2 = concatBytes(encodePktLine("ok refs/heads/main\n"), encodeFlushPkt());

    const data = concatBytes(sideBandPkt(0x01, part1), sideBandPkt(0x01, part2), encodeFlushPkt());
    const result = decodeReceivePackResponse(data);
  ...
  test("过短的 side-band payload 被跳过（不足 2 字节）", () => {
    ...
    const data = concatBytes(sideBandEmptyPkt(0x01), encodeFlushPkt());
    const result = decodeReceivePackResponse(data);
  ...
  test("仅 channel 2 无 channel 1 时 refUpdates 为空", () => {
    const data = concatBytes(sideBandPkt(0x02, "Counting objects\n"), encodeFlushPkt());
    const result = decodeReceivePackResponse(data);

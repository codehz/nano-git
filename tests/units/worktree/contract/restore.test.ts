...
describe("VirtualWorktree contract: restore combinations", () => {
  describe.each(virtualWorktreeBackends)("$name", ({ createWorktree }) => {
    test("restore 递归后立即修改能正确更新 diff", () => {
      ...
      session.writeFile("dir/f.txt", bytes("edited"));
      session.restore("dir", { recursive: true });

      expect(session.diff()).toEqual([]);

      session.writeFile("dir/f.txt", bytes("re-edited"));
    ...
    test("restore 为非递归目录后，子树中写文件仍可读", () => {
      ...
      session.writeFile("dir/a.txt", bytes("edited-a"));
      session.delete("dir/b.txt");
      session.restore("dir");

      expect(bytesToUtf8(session.readFile("dir/a.txt"))).toBe("edited-a");
      expect(session.exists("dir/b.txt")).toBe(false);

      session.writeFile("dir/c.txt", bytes("new-c"));

      expect(bytesToUtf8(session.readFile("dir/c.txt"))).toBe("new-c");
    });

    test("restore 文件前先删除父目录，restore 自动重建目录链", () => {
      const repo = createMemoryRepository();
      const fileHash = repo.writeBlob(bytes("deep-base"));
      ...
      expect(bytesToUtf8(session.readFile("outer/inner/target.txt"))).toBe("deep-base");
      expect(session.diff()).toEqual([]);
    });
    ...
    test("restore 递归后将子树中新增文件标记为删除，diff 正确", () => {
      ...
      session.writeFile("dir/new.txt", bytes("new"));
      session.restore("dir", { recursive: true });
